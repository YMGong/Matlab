...
time_name = {'95to11'};
smb_name = {'_ini','_yr0408','_yr10',''};
beta_name = {'celmer_linear_elevcorr','celmer_linear_noelevcorr','celmer_linear_20thsmb'};
beta_legend_name = {'elevcorr','noelevcorr','90s'};
temp_name = {'temp95'};
...
filename = [E_path_name,'/',base_name_Elmer,'_',time_name,'_',temp_name,'_',cell2mat(beta_name(i)),cell2mat(smb_name(j)),'.ep10.mat'];
%filename_ini = [E_path_name,'/',base_name_Elmer,cell2mat(E_beta_ini_name(i)),'.ep.mat'];
filename_ini = [E_path_name,'/',base_name_Elmer,'_',time_name,'_',temp_name,'_',cell2mat(beta_name(i)),'_ini.ep10.mat'];
...
Elmer_smb = matrixdata(10).data_layer(1).data;
Elmer_smb = (Elmer_smb/1000).*1e3; 
...
Elmer_fVel_change = fVel_Elmer;% - fVel_ini_Elmer;
 ...
 [ArrayX(2).data,ArrayY(2).data,ArrayC(2).data]=improfile(Elmer_fVel_change,eval(xrange),...
    eval(yrange),'bicubic');
%  [ArrayX(3).data,ArrayY(3).data,ArrayC(3).data]=improfile(Elmer_smb,eval(xrange),...
%     eval(yrange),'bicubic'); 
 %AXL(counter) = plot(ArrayX(3).data,ArrayC(3).data,cell2mat(['-',smb_color(i)]),'LineWidth',1.5);
 % AXL(counter) = plot(ArrayX(3).data,ArrayC(3).data,':','LineWidth',2);hold on;
 ...
 [AX,Hplot1(counter).data,Hplot2(counter).data] = plotyy(ArrayX(2).data,ArrayC(2).data,ArrayX(1).data,ArrayC(1).data,'plot');
    ...
    legend_vel(counter) = {['|u|_{',beta_legend_name_input,'}']};
      %  legend_smb(counter) = {['SMB_{',beta_legend_name_input,'}']};
    counter = counter + 1;
...
if 0
   ...
   LH=legend(get(figure(1),'CurrentAxes'),[Hfill;...
       ...
       cell2mat(legend_FS(10)),cell2mat(legend_FS(11)),cell2mat(legend_FS(12)),...
        'sea level',...
       'Location',legend_posi);
       %cell2mat(legend_smb(10)),cell2mat(legend_smb(11)),cell2mat(legend_smb(12)),...
      
    set(LH,'color','none','FontSize',50);
...
%plot(0:length(sum_nocorr_smb)-1,smb_20thsum,cell2mat(['-.',smb_color(3)]),'LineWidth',2);hold on;
sum_smb = ((sum_smb.*(1000/910))./nodes).*1e3;
sum_nocorr_smb = ((sum_nocorr_smb.*(1000/910.0))./nodes).*1e3;

%plot(0:length(sum_smb)-1,sum_smb,cell2mat(['.-',smb_color(1)]),'MarkerSize',12,'LineWidth',1.5);hold on;
%plot(0:length(sum_nocorr_smb)-1,sum_nocorr_smb,cell2mat(['-s',smb_color(2)]),'MarkerSize',12,'LineWidth',1.5);hold on;
%plot(0:length(sum_nocorr_smb)-1,smb_20thsum,cell2mat(['-.',smb_color(3)]),'LineWidth',2);hold on;
% plot(0:length(sum_smb)-1,sum_smb,'.-','MarkerSize',12,'LineWidth',1.5,'Color',get(AXL(counter-3),'Color'));hold on;
% plot(0:length(sum_nocorr_smb)-1,sum_nocorr_smb,'-s','MarkerSize',12,'LineWidth',1.5,'Color',get(AXL(counter-2),'Color'));hold on;
% plot(0:length(sum_nocorr_smb)-1,smb_20thsum,'-.','LineWidth',2,'Color',get(AXL(counter-1),'Color'));hold on;
plot(0:length(sum_smb)-1,sum_smb,'.-','MarkerSize',12,'LineWidth',1.5,'Color','r');hold on;
plot(0:length(sum_nocorr_smb)-1,sum_nocorr_smb,'-.','MarkerSize',12,'LineWidth',1.5,'Color','b');hold on;
plot(0:length(sum_nocorr_smb)-1,smb_20thsum,'-s','LineWidth',2,'Color','k');hold on;
set(gca,'YLim',smblimit,'XLim',xlimit,'xtick',[0,48,96,144,192],...
             'xticklabel',['1995Jan';'1999Jan';'2003Jan';'2007Jan';'2011Jan'],...
     'FontSize',15,'ytick',[-630,-340,0,340]);
 ylabel('Mean Nodal SMB(mm w.e.)');
 grid on;
legend('SMB_{elevcorr}','SMB_{noelevcorr}','SMB_{90s}','Location','SouthEast');
